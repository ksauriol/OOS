...
<head>
    ...
    <script src="http://<?=$_GET['base']?>/BankingSystem/js/GPSViewScripts.js"></script>
        ...
        <table id="profiles_table" class="table table-striped table-hover table-condensed table-responsive">
            ...
            foreach ($data['profiles'] as $gps): ?>
                <tr id="profile_<?=$gps->getProfileID()?>" class="profileRow">
                    ...
                    <td id="profile_isLoggedIn_<?=$gps->getProfileID()?>"><?=$gps->isLoggedIn()?></td>
                    <td id="profile_isPasswordChanged_<?=$gps->getProfileID()?>"><?=$gps->isPasswordChanged()?></td>
                </tr><?php
            endforeach; ?>
            </tbody>
        </table><?php
